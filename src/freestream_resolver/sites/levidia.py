...
class LevidiaScraper:
    ...
    def _find_series_url(self, title: str, year: int | None) -> str | None:
        ...
        for query in queries:
            search_url = f"{self.base_link}{self.search_link % query}"
            r = self._client.post(search_url, headers=headers).text
            if "about 0 results" in r.lower():
                continue
            ...
            for href, label in zip(hrefs, labels):
                ...
                if cleaned != title_key and title_key not in cleaned and cleaned not in title_key:
                    continue
                if year and str(year) not in years:
                    continue
                if not href.startswith("http"):
                    href = f"{self.base_link}/{href.lstrip('/')}"
                return href
        return None

    def scrape(self, req: ScrapeRequest) -> list[StreamCandidate]:
        headers = self._headers()
        match_url = self._find_series_url(req.title, req.year)
        if not match_url:
            return []
        ...
        if req.media_type == "tv" and req.season and req.episode:
            ...
            ep_pat = re.compile(rf"s{req.season}e0*{req.episode}(?!\d)", re.I)
